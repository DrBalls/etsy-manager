import { type SellerTaxonomyNode, type TaxonomyNodeProperty } from '../../types';
...
export class TaxonomyAPI {
   ...
   */
  async getTaxonomyNodeProperties(taxonomyId: string | number): Promise<TaxonomyNodeProperty[]> {
   ...
   */
  async searchTaxonomyNodes(query: string): Promise<SellerTaxonomyNode[]> {
   ...
   */
  async getTaxonomyNode(taxonomyId: string | number): Promise<SellerTaxonomyNode | undefined> {
    const allNodes = await this.getSellerTaxonomyNodes();
   ...
   */
  async getChildTaxonomyNodes(parentId: string | number): Promise<SellerTaxonomyNode[]> {
    const allNodes = await this.getSellerTaxonomyNodes();
   ...
   */
  async getTaxonomyPath(taxonomyId: string | number): Promise<SellerTaxonomyNode[]> {
    const allNodes = await this.getSellerTaxonomyNodes();
   ...
   */
  async getRequiredProperties(taxonomyId: string | number): Promise<TaxonomyNodeProperty[]> {
  ...
  async validateListingProperties(
    ...
    for (const prop of properties) {
      const taxonomyProp = taxonomyProperties.find((tp) => tp.property_id === prop.property_id);
      ...
      if (
        taxonomyProp.max_values_allowed &&
        prop.value_ids.length > taxonomyProp.max_values_allowed
      ) {
        invalidProperties.push({
          property_id: prop.property_id,
      ...
      if (taxonomyProp.possible_values && taxonomyProp.possible_values.length > 0) {
        const validValueIds = new Set(taxonomyProp.possible_values.map((v) => v.value_id));
   ...
   */
  async getSuggestedCategories(title: string, tags?: string[]): Promise<SellerTaxonomyNode[]> {
    const allNodes = await this.getSellerTaxonomyNodes();
